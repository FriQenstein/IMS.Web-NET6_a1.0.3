...
namespace IMS.Web.Repositories
{
    public class LeaveRequestRepository : GenericRepository<LeaveRequest>, ILeaveRequestRepository
    {
        ...
        {
            var user = await userManager.GetUserAsync(httpContextAccessor?.HttpContext?.User);    // get the USER info
            
            var leaveAllocation = await leaveAllocationRepository.GetEmployeeAllocation(user.Id, model.LeaveTypeId);    // get the LEAVEALLOCATION info relative to the LEAVETYPEID
            if(leaveAllocation == null)
            {  // if the USER is new and has no days allocated yet, return FALSE
                return false;
            }

            int daysRequested = (int)(model.EndDate.Value - model.StartDate.Value).TotalDays;                           // get the # of days requested by the user
            if(daysRequested > leaveAllocation.NumberOfDays)            // if REQUEST# is > DAYS remaining, return false
            {
                return false;
            }
